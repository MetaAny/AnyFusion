...
`src/execution/subtask-attempt-runner.ts` executes one Kernel-authorized deterministic attempt. If the Executor changes files, it must commit the complete result, merge the current local Project `main`, resolve conflicts, and leave its assigned branch clean; a result that changes no files only needs a clean assigned worktree. A successful primary/correction attempt commits an immutable receipt and exact candidate commit, then moves the Subtask to `awaiting_integration`; it does not publish result, artifacts, handoffs or `done`. Runtime validates branch identity, clean state and `main` ancestry, creates an `awaiting_approval` publication plus exact `repository_promotion` request, and waits for the user. Top-level Task projections and resume controls must surface this as waiting for approval, not as runnable `ready` work; repeated user/Planner continuation does not submit another dispatch. Approval merges the complete branch into Project `main`, atomically publishes completion facts, and removes the Subtask worktree and branch. Denial blocks the Task/Subtask and preserves them. If `main` changed after the request, Runtime parks the stale publication, preserves the worktree, and returns the Subtask for Executor resynchronization and a new approval. Every non-success commits a terminal receipt and returns control to Kernel policy. A first completion-contract failure may receive one same-AgentClass `contract_correction` full retry: a new Executor session reuses the persistent Subtask worktree, rebuilds the ordinary permission/tool/evidence boundaries, and receives the original Subtask context plus a bounded recovery packet containing the violations. It does not receive the malformed raw response as its task input. A second contract failure blocks.
...
The repository-root `anyfusion` command is the sole native startup and bootstrap path. Gateway service mode runs in the foreground through `anyfusion gateway run`; repository-owned PID/log wrappers and generated systemd service launchers do not exist. A server process supervisor may wrap that command without introducing another configuration path. On Windows-hosted Docker, `docker/gateway.ps1` is the thin orchestration wrapper: the Gateway remains PID 1 beneath `tini`, waits for real Feishu WebSocket readiness, and Docker owns restart policy. Feishu Gateway conversations are Application-Shell sessions keyed by an opaque stable hash of `chat_id`; this isolates and resumes Planner context without changing ADR-0011's single-active-Task admission boundary.
...
Manual continuation has one control identity: Planner emits `resume_task` and the slash surface exposes `/task resume <id> [resources...]` for `ready`, `parked`, and `blocked`. The Application Shell derives `fresh`, `resume-parked`, or `resume-blocked`, including blocked-recovery evidence, only from the current durable Task status; callers never select the execution mode. A `ready` Task carrying a durable awaiting-publication projection only surfaces its approval action and is never dispatched again. Kernel rejects resume controls for every other Task status.
...
**Work Graph**:
The sole execution-structure fact for one task generation: a v6 revisioned DAG of capability-minimal Subtasks whose `dependencies` are both topology and typed delivery contracts. A node may complete with or without workspace changes. Every edge has one to twelve keyed `text` or `artifact` items; only published direct-edge handoffs and controlled task evidence enter downstream context. A pure function derives the runnable frontier without persisting an execution layer. Kernel may authorize up to four independent nodes in one deterministic batch; retry, fallback, continuation, merge repair and bounded replans remain Kernel policy.
...
**Completion Protocol**:
The required final Executor response contract: a non-empty Markdown result description followed by exactly one `metaclaw:completion:v4` marker and strict identity-free JSON. Success uses `{}` or an optional `resultFilePaths` array; controlled failure retains the `failure` object. Runtime verifies every declared result file as an existing workspace-relative file, injects the bound schema/Subtask/attempt/WorkUnit identities, acceptance keys and exact outgoing handoffs, and uses the result description for acceptance evidence and text handoffs. Workspace changes are allowed but never required. Runtime still records the authoritative workspace delta and validates the clean Git publication candidate independently of Completion validation. The machine report is stripped from every user-facing and memory-facing result.
