...
- Validates every Subtask through Completion Protocol v4, records the authoritative workspace delta independently, persists clean results and immutable direct-edge handoffs, and allows exactly one Kernel-authorized same-AgentClass full retry after the first contract failure.
