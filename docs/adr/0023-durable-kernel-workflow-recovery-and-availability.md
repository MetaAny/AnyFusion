...
Kernel contract v3 extends the same durable workflow with `permission_requested`, `permission_review_represented`, `permission_resolution_received`, `partition_conflict_observed` and `sandbox_lost`. Permission grants/denials/escalations, partition waits and workspace-attempt recovery are Decision actions, not Runtime policy. Runtime persists and checkpoints before pausing or destroying an attempt; duplicate request/event/apply identities reuse the prior Decision, grant, lease or outbox effect. Startup reconciles Docker labels with SQLite before accepting input and converts missing or leftover sandboxes into normalized events. It also emits `permission_review_represented` for a valid unresolved repository-promotion request when its publication survived into a new Session; Kernel can only re-escalate that exact request from this event, even if other authorization facts exist. The later applied escalation supersedes only the old UI presentation and does not alter authorization or publication identity. ADR-0024 owns the detailed resource, workspace and elevation contracts.
...
Manual Task continuation has one public and Planner control identity: `/task resume <id> [resources...]` and `resume_task` both accept `ready`, `parked`, or `blocked`. The control proposal and command directive do not carry an execution mode. Kernel rejects other durable Task statuses. The Application Shell derives `fresh`, `resume-parked`, or `resume-blocked` and the corresponding recovery evidence from the authorized Task's current persisted status. A `ready` Task with an awaiting-publication projection only re-surfaces the exact approval action and must not dispatch. The execution Runtime retains its status-specific transition handlers; they are implementation details behind this single control contract.

For an `awaiting_approval` publication, `/task resume` may renew only the review presentation of its existing immutable permission request. Review expiry is measured from the latest applied escalation Decision. The new `permission_review_represented` event causally supersedes that Decision while preserving request ID, fingerprint, Task/Subtask/attempt identity, operation, scope, and candidate commit; it never reruns the Executor. If the request row is missing, the Application Shell may restore it only from the exact original `permission_requested` Kernel event after verifying the full publication identity. Missing or mismatched durable identity fails closed. Approval then follows the same publication, artifact, handoff, and Executor-report path as the initial review.
