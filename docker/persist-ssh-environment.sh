...
persist_ssh_environment() {
  ...
  grep -vE "^(OPENAI_API_KEY|OPENAI_BASE_URL|ANTHROPIC_API_KEY|ANTHROPIC_BASE_URL|DEEPSEEK_API_KEY|GOOGLE_GENERATIVE_AI_API_KEY|OPENROUTER_API_KEY|PI_SKIP_VERSION_CHECK|PI_TELEMETRY|METACLAW_HOME|ANYFUSION_PLANNER_HOME|METACLAW_PLANNER_HOME|METACLAW_EXECUTOR_CODEX_HOME|METACLAW_PLANNER_SESSION_DIR|METACLAW_PLANNER_SCHEMA_PATH|METACLAW_PLANNER_WORKDIR|METACLAW_PLANNER_ENV_FILE|METACLAW_PLANNER_COMMAND|METACLAW_PLANNER_TUI_COMMAND|METACLAW_PLANNER_TUI_SOCKET|ANYFUSION_BRIDGE_SOCKET|ANYFUSION_PLANNER_MODE|ANYFUSION_PLANNER_SCHEMA_PATH|METACLAW_CODEX_EXECUTOR_ENV_FILE|METACLAW_PI_EXECUTOR_ENV_FILE|METACLAW_CONTROL_NETWORK|METACLAW_CONTROL_HOST|METACLAW_DOCKER_HOST_PATH_MAP)=" "$environment_file" > "$tmp" 2>/dev/null || true
  for kv in \
    ...
    "METACLAW_CONTROL_NETWORK=${METACLAW_CONTROL_NETWORK:-metaclaw-control}" \
    "METACLAW_CONTROL_HOST=${METACLAW_CONTROL_HOST:-metaclaw-control}" \
    "METACLAW_DOCKER_HOST_PATH_MAP=${METACLAW_DOCKER_HOST_PATH_MAP:-}"
  do
    local key="${kv%%=*}"
    local val="${kv#*=}"
    if [ -n "$val" ]; then
